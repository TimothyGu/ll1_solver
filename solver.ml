...
module Symbol = struct
  module T = struct
    type t = N of NonTerminal.t | T of Terminal.t [@@deriving compare, sexp]
  end
...
module SymbolList = struct
  module T = struct
    type t = Symbol.t list [@@deriving compare, sexp]
  end
...
let is_t = function N _ -> false | T _ -> true

let extract_nt = function N nt -> nt | T _ -> assert false

let extract_name = function N nt -> nt | T t -> t
    ...
    | (nt, rhs) :: tl_prods -> (
        let is_rhs_nullable deps_map = function
          | [] ->
              (true, deps_map)
          | l ->
              if List.exists l ~f:is_t then (false, deps_map)
...
let iterative_updater (deps_map : ('k, 'kcomp, 'k, 'kcomp) set_map)
    (map : ('k, 'kcomp, 'v, 'vcomp) set_map) : ('k, 'kcomp, 'v, 'vcomp) set_map
    =
...
let make_first_set (nullable_map : nullable_map) (prods : production list) :
    terminal_set_map =
  let rec _make_initial (map : terminal_set_map)
...
let rec is_fragment_nullable (nullable_map : nullable_map) = function
  ...
  | T _ :: _ ->
      false
  | N nt :: rest ->
      Map.find_exn nullable_map nt && is_fragment_nullable nullable_map rest

let first_set_of_fragment (nullable_map : nullable_map)
    (first_set_map : terminal_set_map) (frag : symbol list) : terminal_set =
  ...
  let rec _first_set_acc existing = function
    ...
    | T t :: _ ->
        Set.add existing t
    | N nt :: rest ->
        let with_nt_first_set = Set.union existing (first_set_of_nt nt) in
        if is_nullable nt then _first_set_acc with_nt_first_set rest
        else with_nt_first_set
  in
  _first_set_acc (Set.empty (module Terminal)) frag

let make_follow_set (nullable_map : nullable_map)
    (first_set_map : terminal_set_map) (start_symbol : nonterminal)
  ...
  let rec _make_initial (map : terminal_set_map)
    ...
    | (nt, N rhs_nt :: rhs_rest) :: tl_prods ->
        ...
        let deps_map' =
          if is_fragment_nullable rhs_rest then
            add_set_map (module NonTerminal) deps_map ~key:rhs_nt ~data:nt
          else deps_map
        in
        let prods' = (nt, rhs_rest) :: tl_prods in
        _make_initial map' deps_map' prods'
  ...
  let start_map =
    Map.singleton
      (module NonTerminal)
      start_symbol
      (Set.singleton (module Terminal) '$')
  in
  let init_map, deps_map =
    _make_initial start_map (Map.empty (module NonTerminal)) prods
  in
  compute_fixed_point (iterative_updater deps_map) init_map set_map_equal

module ParsingTableEntry = struct
  module T = struct
    type t = NonTerminal.t * Terminal.t [@@deriving compare, sexp]
  end
...
let make_ll1_table (nullable_map : nullable_map)
    (first_set_map : terminal_set_map) (follow_set_map : terminal_set_map)
  ...
  let rec _make_table tbl = function
    ...
    | (nt, rhs) :: rest_prods ->
        let add_rhs_to_entries terminal_set rhs tbl =
          Set.fold terminal_set ~init:tbl ~f:(fun tbl t ->
              add_set_map (module SymbolList) tbl ~key:(nt, t) ~data:rhs)
        in
        let tbl' = add_rhs_to_entries (get_first_set rhs) rhs tbl in
        let tbl'' =
          if is_fragment_nullable nullable_map rhs then
            add_rhs_to_entries (get_follow_set nt) rhs tbl'
          else tbl'
        in
...
let make_rules orig_str : production list =
  ...
  let classify rhs : symbol list =
    if String.equal rhs "ε" then []
      ...
      |> List.rev_map ~f:(fun ch ->
             if Char.equal ch (Char.lowercase ch) then T ch else N ch)
  in
  ...
  |> Sequence.map ~f:(fun (lhs, rhs) ->
         assert (String.length lhs = 1) ;
         (lhs.[0], Nsplit.nsplit rhs ~by:" | "))
