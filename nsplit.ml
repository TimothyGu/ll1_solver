...
let rfind_from str pos sub =
  ...
  if pos + 1 < 0 || pos + 1 > len then assert false; (* invalid_arg "rfind_from"; *)
  (* 0 <= pos + 1 <= length str *)
  if sublen = 0 then Some (pos + 1) else
    (* length sub > 0 *)
      ...
      if i < 0 then None
        ...
        let rec loop ~str ~sub i j =
          if j = sublen then Some i
            ...
            (* 0 <= j < length sub *)
            (* ==> 0 <= i + j < length str *)
            if Char.(String.unsafe_get str (i + j) <> String.unsafe_get sub j)
...
let nsplit str ~by:sep =
  ...
  else if equal_string sep "" then assert false (* invalid_arg "String.nsplit: empty sep not allowed" *)
  else
    (* str is non empty *)
    let seplen = String.length sep in
    let rec aux acc ofs =
      if ofs >= 0 then (
        match rfind_from str ofs sep
        with
        | Some idx -> (* sep found *)
          ...
          then aux (""::acc) (idx - 1)
            ...
            let token = String.sub str ~pos:(end_of_sep + 1) ~len:(ofs - end_of_sep) in
            aux (token::acc) (idx - 1)
        | None -> (* sep NOT found *)
